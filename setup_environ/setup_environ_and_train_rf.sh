...
NOTEBOOK_PATH="code/4_modeling/4_5_hyperparameter_tuning/hyperparameter_tuning_rf.ipynb"
...
if [ "$RUN_AS_DAEMON" = true ]; then
    ...
    poetry run setsid python3 $OUTPUT_NAME.py > logfile_output_rf 2>&1 &
    echo "Script is running in the background. Logs can be found in logfile_output_rf."
else
